...
/**
 * Promise.race는 Promise.all 메소드와 동일하게 프로미스를 요소로 갖는 배열 등의 이터러블을 인수로 전달받는다.
 * Promise.race는 Promise.all 처럼 모든 프로미스가 fulfilled 상태가 되는 것을 기다리는 것이 아니라 가장 먼저 fulfilled 상태가 된 프로미스의 처리 결과를 resolve하는 새로운 프로미스를 반환한다.
 */

Promise.race([requestData1(), requestData2(), requestData3()])
  .then((res) => console.log('race함수는 이렇게 끝난다.', res)) // (1초 이후) race함수는 이렇게 끝난다. 1
  .catch(console.log);

/**
 * 프로미스가 하나라도 rejected 상태가 되면 Promise.all 메소드와 동일하게 처리된다.
 */
